...
class DBEngine:
    ...
    def execute(self, table_id, select_index, aggregation_index, conditions, lower=True):
        if not table_id.startswith('table'):
            table_id = '{}'.format(table_id.replace('-', '_'))
        ...
        for col_index, op, val in conditions:
            if lower and isinstance(val, str):
                val = val.lower()
            if schema[colList[select_index]] == 'real' and not isinstance(val, (int, float)):
                try:
                    val = float(parse_decimal(val))
                except NumberFormatError as e:
                    val = float(num_re.findall(val)[0])
            where_clause.append(f'{colList[select_index]} {Query.cond_ops[op]} :{colList[select_index]}')
            where_map[colList[select_index]] = val
        where_str = ''
        if where_clause:
